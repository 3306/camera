...
namespace ServerDLL
{
    public class Video
    {
        ...
        public void set()
        {
            Capparms.AVStreamMaster = 1;

            Capparms.dwAudioBufferSize = 1024;
        ...
        {
            this.lwndC = AviCapture.capCreateCaptureWindow("222222", AviCapture.WS_CHILD | AviCapture.WS_VISIBLE, 0, 0, mWidth, mHeight, mControlPtr, 0);//AVICap类的捕捉窗口
            ...
            if (!CapDriverCAPS.fCaptureInitialized)//判断当前设备是否被其他设备连接已经连接
            {
                ...
                if (AviCapture.SendMessage(this.lwndC, AviCapture.WM_CAP_DRIVER_CONNECT, 0, 0))
                {
                    ...
                    AviCapture.SendMessage(this.lwndC, AviCapture.WM_CAP_SET_PREVIEWRATE, 50, 0);//设置在PREVIEW模式下设定视频窗口的刷新率 设置每34毫秒显示一帧，即显示帧速为每秒29帧
        ...
        public void kkkk()
        {
            AviCapture.SendMessage(lwndC, AviCapture.WM_CAP_DLG_VIDEODISPLAY, 1, 1);
        }
